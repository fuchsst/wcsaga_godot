class_name NotEqualsFunction
extends BaseSexpFunction

## Not equals comparison operator for SEXP expressions
##
## Usage: (!= value1 value2 value3 ...)
## Returns: Boolean result - true if first value is not equal to ANY other value

func _init():
	super._init("!=", "comparison", "Not equals - returns true if first argument is not equal to any other")
...
func _execute_implementation(args: Array[SexpResult]) -> SexpResult:
	...
	# Check if first argument is NOT equal to any other argument
	for i in range(1, args.size()):
		...
		if comparison_result != 0:  # Found a not-equal value
			return SexpResult.create_boolean(true)
	
	# All values were equal to the first
...
func get_usage_examples() -> Array[String]:
	return [
		"(!= 5 3) ; Returns true",
		"(!= 5 5) ; Returns false (values are equal)",
		"(!= 5 5 3) ; Returns true (5 != 3)",
		"(!= 5 5 5) ; Returns false (all values equal)"
	]