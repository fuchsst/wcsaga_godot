class_name GreaterThanFunction
extends BaseSexpFunction

## Greater than comparison operator for SEXP expressions
##
## Implements the greater than operator following WCS semantics.
## Compares the first argument against ALL remaining arguments.
## Returns true only if the first argument is greater than ALL other arguments.
##
## Usage: (> value1 value2 value3 ...)
## Returns: Boolean result - true if first value is greater than all others

func _init():
	super._init(">", "comparison", "Greater than - returns true if first argument is greater than all others")
...
func _execute_implementation(args: Array[SexpResult]) -> SexpResult:
	...
	for i in range(1, args.size()):
		...
		if comparison_result <= 0:  # Not greater than
			return SexpResult.create_boolean(false)
	
	# First argument was greater than all others
...
func _compare_values(left: SexpResult, right: SexpResult) -> int:
	...
	match left.result_type:
		...
		_:
			# Non-comparable types default to not-greater-than
			return 0
...
func get_usage_examples() -> Array[String]:
	return [
		...
		"(> \"b\" \"a\") ; Returns true",
		"(> \"10\" \"5\") ; Returns false (lexicographic: \"10\" < \"5\")",
		"(> 10 \"5\") ; Returns true (numeric comparison: 10 > 5)"
	]
...
It returns true only if the first argument is greater than ALL other arguments.
