class_name LessThanOrEqualFunction
extends BaseSexpFunction

## Less than or equal comparison operator for SEXP expressions
##
## Usage: (<= value1 value2 value3 ...)
## Returns: Boolean result - true if first value is less than or equal to all others

func _init():
	super._init("<=", "comparison", "Less than or equal - returns true if first argument is <= all others")
...
func _execute_implementation(args: Array[SexpResult]) -> SexpResult:
	...
	for i in range(1, args.size()):
		...
		if comparison_result > 0:  # Not less than or equal
...
func _compare_values(left: SexpResult, right: SexpResult) -> int:
	# Same comparison logic as other operators
