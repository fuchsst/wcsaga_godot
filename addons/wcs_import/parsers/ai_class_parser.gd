...
func _parse_multi_value_parameter(class_resources: Array[AIClassResource], line: String) -> void:
	...
	if values_str.contains(","):
		...
		if value_parts.size() != 5:
			push_warning("Expected 5 values for parameter '" + param_name + "', got " + str(value_parts.size()))
			return
...
func _set_property_value(ai_class: AIClassResource, property_name: String, value_str: String, param_name: String, log_warning: bool) -> void:
	...
	if value_str.to_upper() == "YES" or value_str.to_upper() == "NO":
		...
		if property_name in ai_class:
			...
			push_warning("Unknown boolean parameter: " + param_name + " (property: " + property_name + ")")
		...
		if property_name in ai_class:
			...
			push_warning("Unknown numeric parameter: " + param_name + " (property: " + property_name + ")")
