...
func _parse_ship(first_line: String) -> ShipStats:
	...
	while _has_more_lines():
		...
		if line.begins_with("$Short name:"):
			...
			if "fighter" in type_str: ship.ship_role = 0
			elif "bomber" in type_str: ship.ship_role = 1
			elif "cruiser" in type_str or "corvette" in type_str or "destroyer" in type_str or "capital" in type_str: ship.ship_role = 2
			elif "transport" in type_str or "freighter" in type_str: ship.ship_role = 3
