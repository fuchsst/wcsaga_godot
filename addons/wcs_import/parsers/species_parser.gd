		...
		if line.begins_with("$Species_Name:"):
			...
			if line.begins_with("$Default IFF:"):
				...
				current_species.set_meta(
					"debris_texture_filename", _extract_string_value(line, "+Debris_Texture:")
				)
			elif line.begins_with("+Shield_Hit_ani:"):
				current_species.set_meta(
					"shield_hit_anim_filename", _extract_string_value(line, "+Shield_Hit_ani:")
				)

			# Thruster Anims
			elif line.begins_with("+Pri_Normal:"):
				current_species.set_meta(
					"thruster_normal_filename", _extract_string_value(line, "+Pri_Normal:")
				)
			elif line.begins_with("+Pri_Afterburn:"):
				current_species.set_meta(
					"thruster_afterburn_filename", _extract_string_value(line, "+Pri_Afterburn:")
				)
			elif line.begins_with("+Sec_Normal:"):
				current_species.set_meta(
					"thruster_secondary_normal_filename",
					_extract_string_value(line, "+Sec_Normal:")
				)
			elif line.begins_with("+Sec_Afterburn:"):
				current_species.set_meta(
					"thruster_secondary_afterburn_filename",
					_extract_string_value(line, "+Sec_Afterburn:")
				)
			elif line.begins_with("+Ter_Normal:"):
				current_species.set_meta(
					"thruster_tertiary_normal_filename", _extract_string_value(line, "+Ter_Normal:")
				)
			elif line.begins_with("+Ter_Afterburn:"):
				current_species.set_meta(
					"thruster_tertiary_afterburn_filename",
					_extract_string_value(line, "+Ter_Afterburn:")
				)

			# Thruster Glows
			elif line.begins_with("+Normal:"):
				current_species.set_meta(
					"glow_normal_filename", _extract_string_value(line, "+Normal:")
				)
			elif line.begins_with("+Afterburn:"):
				current_species.set_meta(
					"glow_afterburn_filename", _extract_string_value(line, "+Afterburn:")
				)
