...
func _parse_weapon(first_line: String) -> Resource:
	...
	while _has_more_lines():
		...
		if line.begins_with("+Title:"):
			...
			if homing_val == "YES":
				weapon.homing_type = WeaponDataScript.HomingType.ASPECT
			elif homing_val == "NO":
				weapon.homing_type = WeaponDataScript.HomingType.NONE
			elif homing_val == "ASPECT":
				weapon.homing_type = WeaponDataScript.HomingType.ASPECT
			elif homing_val == "HEAT":
				weapon.homing_type = WeaponDataScript.HomingType.HEAT
			...
			if weapon.swarm_config == null:
				weapon.swarm_config = WeaponDataScript.SwarmConfiguration.new()
...
func _parse_flags(line: String, weapon) -> void:
	...
	for flag in flags_list:
		match flag.to_lower():
			...
			"stream":
				weapon.fire_rate_hz = 10.0
			"no pierce shields":
				weapon.no_shield_piercing = true
			"bomb":
			...
			"particle spew":
				...
				if weapon.particle_spew == null:
					weapon.particle_spew = WeaponDataScript.ParticleSpew.new()
			"ballistic":
				weapon.flags |= WeaponDataScript.WeaponFlags.BALLISTIC
			"swarm":
				weapon.flags |= WeaponDataScript.WeaponFlags.SWARM
			"corkscrew":
				weapon.flags |= WeaponDataScript.WeaponFlags.CORKSCREW
			"flak":
				weapon.flags |= WeaponDataScript.WeaponFlags.FLAK
			"electronics":
				weapon.flags |= WeaponDataScript.WeaponFlags.ELECTRONICS
			"spawn child":
				weapon.flags |= WeaponDataScript.WeaponFlags.SPAWN_CHILD
			"remote detonate":
				weapon.flags |= WeaponDataScript.WeaponFlags.REMOTE_DETONATE
			"puncture":
				weapon.flags |= WeaponDataScript.WeaponFlags.PUNCTURE
			"shield pierce":
				weapon.flags |= WeaponDataScript.WeaponFlags.SHIELD_PIERCE
			"bomber+":
...
func _extract_color_value(line: String, prefix: String) -> Color:
	...
	if parts.size() >= 3:
		return Color(
			parts[0].to_float() / 255.0, parts[1].to_float() / 255.0, parts[2].to_float() / 255.0
		)
	return Color.WHITE
...
func _parse_corkscrew_info(weapon) -> void:
	...
	while _has_more_lines():
		...
		if line.begins_with("+Num:"):
			...
			weapon.corkscrew_config.counter_rotate = (
				line.to_lower().contains("yes") or line.to_lower().contains("true")
			)
		elif line.begins_with("+Helix:"):
			weapon.corkscrew_config.helix = (
				line.to_lower().contains("yes") or line.to_lower().contains("true")
			)
...
func _determine_faction_path(weapon_name: String) -> Dictionary:
	...
	# Prefixes
	if name.begins_with("kif_"):
		return {"category": "fighter", "faction": "kilrathi"}
	if name.begins_with("kib_"):
		return {"category": "bomber", "faction": "kilrathi"}
	if name.begins_with("kim_"):
		return {"category": "missile", "faction": "kilrathi"}
	if name.begins_with("kis_"):
		return {"category": "capital", "faction": "kilrathi"}
	if name.begins_with("kb_"):
		return {"category": "station", "faction": "kilrathi"}

	if name.begins_with("tcf_"):
		return {"category": "fighter", "faction": "terran"}
	if name.begins_with("tcb_"):
		return {"category": "bomber", "faction": "terran"}
	if name.begins_with("tcm_"):
		return {"category": "missile", "faction": "terran"}
	if name.begins_with("tcs_"):
		return {"category": "capital", "faction": "terran"}
	if name.begins_with("tb_"):
		return {"category": "station", "faction": "terran"}

	if name.begins_with("prf_"):
		return {"category": "fighter", "faction": "pirate"}
	if name.begins_with("prs_"):
		return {"category": "capital", "faction": "pirate"}

	# Special cases
	if name == "stormfire":
		return {"category": "weapon", "faction": "special"}
	if name == "infyrno":
		return {"category": "weapon", "faction": "special"}
