...
func _parse_content() -> Variant:
	...
	while _has_more_lines():
		...
		if (
			line.begins_with("#AI Profiles")
			or line.begins_with("$Default Profile:")
			or line.begins_with("$Profile Name:")
		):
			continue
...
func _parse_multi_value_flag(profiles: Array[AIProfileResource], line: String) -> void:
	...
	if values_str.contains(","):
		...
		if value_parts.size() != 5:
			push_warning(
				"Expected 5 values for flag '" + flag_name + "', got " + str(value_parts.size())
			)
			return
...
func _set_property_value(
	...
	if value_str.to_upper() == "YES" or value_str.to_upper() == "NO":
		...
		if property_name in profile:
			...
			push_warning(
				"Unknown boolean flag: " + flag_name + " (property: " + property_name + ")"
			)
		...
		if property_name in profile:
			...
			push_warning(
				"Unknown numeric flag: " + flag_name + " (property: " + property_name + ")"
			)
