...
enum WeaponType {ENERGY = 0, BALLISTIC = 1, MISSILE = 2, SPECIAL = 3}

enum HomingType {NONE = 0, ASPECT = 1, HEAT = 2, IMAGE = 3, FRIEND_FOE = 4}
...
@export var laser_bitmap: Texture2D # Cross-reference to 2D texture (converted from _laser_bitmap_source)
@export var laser_glow: Texture2D # Cross-reference to glow effect (converted from _laser_glow_source)
var _laser_bitmap_source: String = "" # TBL source name
var _laser_glow_source: String = "" # TBL source name
...
func _validate_visual_effects() -> void:
	...
	var effect_refs = [
		projectile_model,
		# laser_bitmap and laser_glow are now Texture2D, validated separately if needed
		muzzle_flash_effect,
		impact_effect,
		explosion_effect,
	...
	if laser_bitmap == null and not _laser_bitmap_source.is_empty():
		_add_validation_warning("Laser bitmap source defined but texture not loaded: " + _laser_bitmap_source)
...
func calculate_damage_against_target(
	...
	result["total_damage"] = (
		result["shield_damage"] + result["armor_damage"] + result["hull_damage"]
	)
...
func calculate_explosion_damage(target_position: Vector3, explosion_radius: float) -> Dictionary:
	...
	var result = {
		"explosion_damage": 0.0, "armor_damage": 0.0, "hull_damage": 0.0, "shield_damage": 0.0
	}
