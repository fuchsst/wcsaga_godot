...
func _validate_severe_environmental_effects() -> void:
	...
	if has_cloaking_technology and nebula_classification != 2:
		_add_validation_warning(
			"Advanced stealth technology may not be appropriate for all nebula types"
		)
...
func calculate_tactical_severity() -> float:
	...
	if actively_drains_shields:
		severe_penalty += 0.3
	if blocks_beam_weapons:
		severe_penalty += 0.25
	if disrupts_navigation_systems:
		severe_penalty += 0.2
	if disables_shield_generators:
		severe_penalty += 0.35
...
func is_safe_for_prolonged_exposure() -> bool:
	...
	return (
		continuous_radiation_damage < 0.5
		and shield_drain_rate_per_second < 1.0
		and electrical_malfunction_rate < 0.02
		and calculate_tactical_severity() < 0.4
		and not causes_sensor_blackout
	)
...
func get_performance_impact_estimate() -> Dictionary:
	...
	return {
		...
		"category":
		"Moderate" if (cpu_performance_impact + gpu_performance_impact) < 0.7 else "High",
		"recommended_quality":
		"High" if (cpu_performance_impact + gpu_performance_impact) < 0.5 else "Medium"
	}
		...
		"Sniper Mission":
			if radar_sensor_range_reduction > 0.5 or weapon_range_modifier < 0.8:
				...
				compatibility["warnings"].append(
					"Range and sensors too compromised for sniper role"
				)
