...
func update_from_game_state() -> void:
	# Check if GameManager exists (assuming it's an Autoload)
	if Engine.has_singleton("GameManager"):
		var game_manager = Engine.get_singleton("GameManager")
		# Update gauge properties using GameManager methods
		# Setters handle queue_redraw
		mission_time = game_manager.get_mission_time()
		time_compression = game_manager.get_time_compression()
	else:
		# Default state if no GameManager
		mission_time = 0.0
		time_compression = 1.0

# Update mission time (This might become obsolete if update_from_game_state is used)
