...
func update_from_game_state() -> void:
	...
	if GameStateManager.player_ship and is_instance_valid(GameStateManager.player_ship):
		...
		if threat_data:
			...
			# { direction: Vector2, distance: float, threat_level: float }
			# Direction should be relative to the player's view/ship forward direction
			# Threat level could be based on missile type, distance, aspect lock, etc.
			show_warning(
				threat_data.get("direction", Vector2.UP), # Default UP if missing
				threat_data.get("distance", 0.0),
				threat_data.get("threat_level", 1.0)
			)
		else:
			clear_warning()
	else:
		# No player ship, clear warning
		clear_warning()
