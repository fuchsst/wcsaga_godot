...
func update_from_game_state() -> void:
	...
	if GameStateManager.player_ship and is_instance_valid(GameStateManager.player_ship) and GameStateManager.player_ship.targeting_component:
		...
		if has_target:
			target_distance = targeting_comp.get_target_distance() # Placeholder method
			# TODO: Determine target_in_range based on selected weapon range
			# Needs access to WeaponSystem and WeaponData
			target_in_range = targeting_comp.is_target_in_primary_range() # Placeholder method

			# Update lead position (needs calculation based on target/weapon)
			# This calculation might live in TargetingComponent or be done here
			lead_position = targeting_comp.get_lead_indicator_position_2d() # Placeholder method
		else:
			target_distance = 0.0
			target_in_range = false
			lead_position = Vector2.ZERO

		# Update lock status
		var current_lock_state = targeting_comp.get_lock_state() # Placeholder method returning LockState enum
		var current_lock_progress = targeting_comp.get_lock_progress() # Placeholder method returning 0.0-1.0
		...
		# Default state if no player ship or targeting component
		has_target = false
		target_in_range = false
		target_distance = 0.0
		lead_position = Vector2.ZERO
