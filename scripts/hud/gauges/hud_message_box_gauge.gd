...
func update_from_game_state() -> void:
	# This gauge likely displays specific, non-scrolling messages.
	# We need a way to determine which message (if any) should be shown here.
	# This might involve checking a dedicated queue in MessageManager,
	# or specific flags in GameState or TrainingManager.

	# Placeholder Logic: Assume a function exists to get the current message for this box.
	var message_data = null # Placeholder: MessageManager.get_current_message_for_box()

	if message_data:
		# Check if this message is already displayed or if it's different
		if messages.is_empty() or messages[0].text != message_data.get("text", ""):
			clear_messages() # Clear previous message if different
			add_message(
				message_data.get("text", ""),
				message_data.get("type", MessageType.NORMAL),
				message_data.get("duration", default_duration),
				message_data.get("source", "")
			)
	else:
		# No message for this box currently, ensure it's clear (handled by _process)
		pass
