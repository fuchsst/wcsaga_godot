...
func update_from_game_state() -> void:
	...
	if Engine.has_singleton("MessageManager"):
		...
		# Assuming MessageManager has a method to get the current talking head info
		var head_data = message_manager.get_current_talking_head_data() # Placeholder method

		if head_data:
			# Assuming head_data is a Dictionary like:
			# { anim_name: "...", source_name: "...", duration: 5.0 }
			var anim_name = head_data.get("anim_name", "default")
			var source = head_data.get("source_name", "")
			var duration = head_data.get("duration", message_duration)

			# Check if this animation is already playing or if it's different
			if not _is_playing or current_animation != anim_name or source_name != source:
				# TODO: Need to load the correct SpriteFrames resource based on anim_name
				# This might involve a lookup table or resource path convention.
				# For now, assume animation_frames is somehow set correctly externally or before calling play.
				# Example: animation_frames = load("res://assets/animations/" + anim_name + ".tres")
				play_message(anim_name, source, duration)
		else:
			# No active talking head message
			if _is_playing:
				stop_message()
	else:
		# No MessageManager, ensure stopped
		if _is_playing:
			stop_message()
