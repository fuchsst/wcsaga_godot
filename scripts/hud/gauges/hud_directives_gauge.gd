...
func update_from_game_state() -> void:
	# Check if MissionManager exists (assuming it's an Autoload or accessible)
	if Engine.has_singleton("MissionManager"):
		var mission_manager = Engine.get_singleton("MissionManager")
		# Assuming MissionManager has a method to get currently relevant objectives
		# This method should return data compatible with the Objective inner class
		# or we need to adapt the data here.
		var current_mission_objectives = mission_manager.get_hud_objectives() # Placeholder method

		var new_objectives_data = []
		var updated = false

		for obj_data in current_mission_objectives:
			# Assuming obj_data is a Dictionary like:
			# { text: "Destroy Base", type: ObjectiveType.PRIMARY, status: ObjectiveStatus.INCOMPLETE, total: 1, current: 0 }
			var new_obj = Objective.new(
				obj_data.get("text", ""),
				obj_data.get("type", ObjectiveType.PRIMARY),
				obj_data.get("status", ObjectiveStatus.INCOMPLETE),
				obj_data.get("total", 1),
				obj_data.get("current", 0)
			)
			new_objectives_data.append(new_obj)

			# Check if this objective is new or its status changed to trigger notification
			var found_existing = false
			for existing_obj in objectives:
				# Need a reliable way to identify objectives (e.g., an ID or comparing text)
				if existing_obj.text == new_obj.text: # Simple text comparison for now
					found_existing = true
					if existing_obj.status != new_obj.status or existing_obj.current_count != new_obj.current_count:
						if not new_obj in _notify_objectives:
							new_obj.notify_time = Time.get_ticks_msec() / 1000.0
							_notify_objectives.append(new_obj)
							updated = true # Mark for redraw due to notification change
					break
			if not found_existing:
				# New objective added
				new_obj.notify_time = Time.get_ticks_msec() / 1000.0
				_notify_objectives.append(new_obj)
				updated = true # Mark for redraw

		# Check if the list itself changed (size or order might differ)
		if objectives.size() != new_objectives_data.size():
			updated = true
		else:
			# Quick check if status or counts changed for existing text matches
			# A more robust check would compare all fields or use IDs
			pass # Already handled notification check above

		# Update the gauge's objectives array if changed
		if updated:
			objectives = new_objectives_data # Setter handles queue_redraw
		elif not objectives.is_empty() and objectives[0].text != new_objectives_data[0].text:
			# Fallback redraw if order changed but status didn't trigger notification check
			objectives = new_objectives_data

	else:
		# Default state if no MissionManager
		objectives = []
