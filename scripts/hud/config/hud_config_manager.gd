...
signal config_loaded(config: HUDConfigExtended)
...
# Configuration state
var current_config: HUDConfigExtended
var default_config: HUDConfigExtended
...
func _load_default_config() -> void:
	if ResourceLoader.exists(default_config_path):
		default_config = ResourceLoader.load(default_config_path) as HUDConfigExtended
		if default_config:
...
func _create_fallback_config() -> void:
	default_config = HUDConfigExtended.new()
	...
	default_config.gauge_visibility_flags = HUDConfigExtended.DEFAULT_FLAGS
...
func _load_user_config(profile_name: String = "default") -> bool:
	...
	if loaded_config:
		...
		# Use default config
		current_config = default_config.duplicate() if default_config else HUDConfigExtended.new()
		active_profile_name = profile_name
		_apply_loaded_config()
...
func import_config(file_path: String, profile_name: String) -> bool:
	...
	var imported_config = ResourceLoader.load(file_path) as HUDConfigExtended
