...
var _lightning_bolts: Array[MeshInstance3D] = []  # Active bolt meshes
var _lightning_timers: Array[float] = []  # Bolt fade timers
const LIGHTNING_DURATION: float = 0.25  # How long bolt is visible
const LIGHTNING_SEGMENTS: int = 8  # Jaggedness
const MAX_LIGHTNING_BOLTS: int = 3
...
func initialize_from_resource(data: Resource) -> void:
	...
	_fog_color = (
		data.get("nebula_tint_color")
		if data.get("nebula_tint_color")
		else Color(0.3, 0.2, 0.4, 0.5)
	)
	_fog_density = (
		data.get("visual_opacity_coefficient") if data.get("visual_opacity_coefficient") else 0.3
	)
	_particle_velocity = (
		data.get("particle_velocity_vector")
		if data.get("particle_velocity_vector")
		else Vector3.ZERO
	)
	...
	if data.get("nebula_classification") == 1:  # Ion Storm
		_lightning_interval = (
			3.0
			/ max(data.get("ion_storm_intensity") if data.get("ion_storm_intensity") else 0.1, 0.01)
		)
	else:
	...
	print(
		(
			"NebulaController: Initialized nebula '%s' (Classification: %s)"
			% [
				data.get("nebula_name") if data.get("nebula_name") else "unnamed",
				_get_classification_name(
					data.get("nebula_classification") if data.get("nebula_classification") else 0
				)
			]
		)
	)
...
func _setup_particles() -> void:
	...
	if nebula_data:
		particle_density = (
			nebula_data.get("particle_density_per_cubic_meter")
			if nebula_data.get("particle_density_per_cubic_meter")
			else 50
		)
		turbulence = (
			nebula_data.get("turbulence_force_factor")
			if nebula_data.get("turbulence_force_factor")
			else 0.2
		)
	...
	material.direction = (
		_particle_velocity.normalized() if _particle_velocity.length() > 0 else Vector3(1, 0, 0)
	)
...
func _setup_background() -> void:
	...
	for i in range(directions.size()):
		...
		sprite.pixel_size = (
			size / background_texture.get_width() if background_texture.get_width() > 0 else 1.0
		)
...
func _update_lightning(delta: float) -> void:
	...
	# Update existing bolts (fade and cleanup)
	...
	if _lightning_timer >= _lightning_interval:
		...
		var ion_intensity: float = (
			nebula_data.get("ion_storm_intensity")
			if nebula_data.get("ion_storm_intensity")
			else 0.1
		)
...
func _spawn_lightning_bolt() -> void:
	"""Create a visible lightning bolt mesh"""
	var camera := get_viewport().get_camera_3d()
	if not camera:
		return

	# Limit active bolts
	if _lightning_bolts.size() >= MAX_LIGHTNING_BOLTS:
		var old_bolt: MeshInstance3D = _lightning_bolts.pop_front()
		_lightning_timers.pop_front()
		if is_instance_valid(old_bolt):
			old_bolt.queue_free()

	# Generate random bolt position relative to camera
	var cam_pos := camera.global_position
	var start := (
		cam_pos + Vector3(randf_range(-200, 200), randf_range(50, 150), randf_range(-200, 200))
	)
	var end_pt := (
		start + Vector3(randf_range(-50, 50), randf_range(-200, -100), randf_range(-50, 50))
	)

	# Create mesh instance
	var mesh_inst := MeshInstance3D.new()
	mesh_inst.name = "LightningBolt"
	add_child(mesh_inst)

	# Create unshaded emission material
	var mat := StandardMaterial3D.new()
	mat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	mat.albedo_color = Color(0.8, 0.9, 1.0)  # Bright white-blue
	mat.emission_enabled = true
	mat.emission = Color(0.6, 0.8, 1.0)
	mat.emission_energy_multiplier = 5.0
	mesh_inst.material_override = mat

	# Build jagged bolt using ImmediateMesh
	var im := ImmediateMesh.new()
	im.surface_begin(Mesh.PRIMITIVE_LINES)

	var prev := start
	for i in range(LIGHTNING_SEGMENTS):
		var t := float(i + 1) / float(LIGHTNING_SEGMENTS)
		var next := start.lerp(end_pt, t)

		# Add random offset (except for last point)
		if i < LIGHTNING_SEGMENTS - 1:
			var offset := Vector3(
				randf_range(-15.0, 15.0), randf_range(-5.0, 5.0), randf_range(-15.0, 15.0)
			)
			next += offset
	...
	# Track for cleanup
	_lightning_bolts.append(mesh_inst)
	_lightning_timers.append(LIGHTNING_DURATION)


func _update_lightning_bolts(delta: float) -> void:
	"""Update bolt timers and cleanup expired bolts"""
	var i := 0
	while i < _lightning_timers.size():
		_lightning_timers[i] -= delta

		# Fade out opacity
		if i < _lightning_bolts.size() and is_instance_valid(_lightning_bolts[i]):
			var bolt := _lightning_bolts[i]
			var mat: StandardMaterial3D = bolt.material_override
			if mat:
				var alpha := clampf(_lightning_timers[i] / LIGHTNING_DURATION, 0.0, 1.0)
				mat.albedo_color.a = alpha
				mat.emission_energy_multiplier = 5.0 * alpha

		if _lightning_timers[i] <= 0:
			if i < _lightning_bolts.size() and is_instance_valid(_lightning_bolts[i]):
				_lightning_bolts[i].queue_free()
			_lightning_bolts.remove_at(i)
			_lightning_timers.remove_at(i)
		else:
			i += 1
...
func calculate_ship_impact(ship_stats: Resource) -> Dictionary:
	...
	return {
		"velocity_impact":
		(
			nebula_data.get("max_velocity_reduction")
			if nebula_data.get("max_velocity_reduction")
			else 0.0
		),
		"sensor_impact":
		(
			nebula_data.get("radar_sensor_range_reduction")
			if nebula_data.get("radar_sensor_range_reduction")
			else 0.0
		),
		"shield_impact":
		(
			1.0
			- (
				nebula_data.get("shield_effectiveness_multiplier")
				if nebula_data.get("shield_effectiveness_multiplier")
				else 1.0
			)
		)
	}
...
func get_classification_name() -> String:
	if not nebula_data:
		return "Unknown"
	return _get_classification_name(
		nebula_data.get("nebula_classification") if nebula_data.get("nebula_classification") else 0
	)
