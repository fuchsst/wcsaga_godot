...
func configure_from_mission(mission_data: Resource) -> void:
	...
	print("EnvironmentManager: Environment configured with %d suns, nebula: %s" % [
		suns.size(),
		"yes" if nebula else "no"
	])
...
func _extract_environment_config(mission_data: Resource) -> Dictionary:
	...
	if background_data:
		...
		for sun_entry in suns_data:
			...
			var sun_angles: Vector3 = sun_entry.get("angles") if sun_entry.get("angles") else Vector3.ZERO
			sun_list.append({
		...
		var background_sets: Array = background_data.get("background_sets") if background_data.get("background_sets") else []
		...
		if nebula_data:
			var nebula_type: String = nebula_data.get("nebula_type") if nebula_data.get("nebula_type") else ""
...
func get_environment_report() -> Dictionary:
	...
	if nebula:
		report["nebula_name"] = nebula.nebula_data.get("nebula_name") if nebula.nebula_data else "Unknown"
