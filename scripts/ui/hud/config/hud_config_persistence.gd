...
signal config_loaded(profile_name: String, config: HUDConfig)
...
func load_config(profile_name: String = "default") -> HUDConfig:
	...
	var config = ResourceLoader.load(file_path) as HUDConfig
	if not config:
...
func _migrate_from_0_9_0(config: HUDConfig) -> HUDConfig:
...
func _migrate_from_0_9_5(config: HUDConfig) -> HUDConfig:
...
func _load_backup(profile_name: String) -> HUDConfig:
	var backup_path = _get_backup_file_path(profile_name)
	
	if not FileAccess.file_exists(backup_path):
		return null
	
	var config = ResourceLoader.load(backup_path) as HUDConfig
	return config
...
func export_config(config: HUDConfig, export_path: String) -> bool:
	if not config:
...
func import_config(import_path: String, profile_name: String) -> bool:
	...
	var config = ResourceLoader.load(import_path) as HUDConfig
	if not config:
