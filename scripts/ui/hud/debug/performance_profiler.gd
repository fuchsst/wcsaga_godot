...
func _find_node_by_class_name(node: Node, class_name: String) -> Node:
	var script = node.get_script()
	if script and script.get_path().ends_with(".gd"):
		# Check if the node has the class_name we're looking for
		if node.has_method("get_script") and str(script).contains(class_name):
			return node
	
	for child in node.get_children():
		var result = _find_node_by_class_name(child, class_name)
		if result:
			return result
