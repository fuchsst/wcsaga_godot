...
func _test_tracker_initialization(tracker: MultiTargetTracker, result: Dictionary) -> bool:
	if tracker == null:
		result.details.append("ERROR: Tracker instance is null")
		return false
	if tracker.max_tracked_targets != 32:
		result.details.append("ERROR: Default max targets should be 32")
		return false
	if not tracker.active_tracks.is_empty():
		result.details.append("ERROR: Should have no active tracks initially")
		return false
	if tracker.target_priority_manager == null:
		result.details.append("ERROR: Priority manager not initialized")
		return false
	if tracker.tracking_radar == null:
		result.details.append("ERROR: Tracking radar not initialized")  
		return false
	if tracker.threat_assessment == null:
		result.details.append("ERROR: Threat assessment not initialized")
		return false
	if tracker.target_classification == null:
		result.details.append("ERROR: Target classification not initialized")
		return false
	if tracker.tracking_database == null:
		result.details.append("ERROR: Tracking database not initialized")
		return false
	if tracker.situational_awareness == null:
		result.details.append("ERROR: Situational awareness not initialized")
		return false
	if tracker.target_handoff == null:
		result.details.append("ERROR: Target handoff not initialized")
		return false
